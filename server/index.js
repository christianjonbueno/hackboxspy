...
//   console.log("New client connected", playerData)
...
io.on('connection', socket => {
  ...
  socket.on('ready', data => {
    ...
    if (data.id == currentSpy[data.room].id) {
      ...
      console.log(randPic)
      console.log('you are not the spy')
      ...
      if (count[data.room] === 4) {
        io.to(data.room).emit('startButton', {messaage:'The game will start once any player presses Begin.'})
        count[data.room] = 0;
      }
    }
  })
